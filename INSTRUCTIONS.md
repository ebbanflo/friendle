...
`data/solutions.js` (~10k) and `data/guesses.js` (~14.8k) are generated by
`tools/build-words.mjs` from six public sources (Wordle valid-guess list,
Knuth's five-letter words, dwyl dictionary, Google Books + OpenSubtitles
frequency corpora, LDNOOBW profanity list — URLs in the script header).
Candidates are evidence-ranked across sources; profanity and slurs are
stripped from both lists (plus a solutions-only exclusion tier for dual-use
words), and solutions ship **ordered commonest-first** — `words.js` squares
the random variate so everyday words dominate play while the deep tail keeps
a marathon Royale from repeating (used words are tracked per session).
